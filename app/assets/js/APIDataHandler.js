class APIDataHandler {
    ...
    checkFilters(witchProperty, filterProperty) {
        ...
        if (Array.isArray(witchProperty)) {
            witchProperty.map((filter) => {
                this.filterProperties[filterProperty[0]][filter] =
                    this.getFilterObject(filter, found, filterProperty[1])
            })
        } else {
    ...
    getWitchNonOptionalProperties(item) {
        ...
        let investigationDate = item.hasOwnProperty('investigationStart')
            ...
            : 'N/A'
        let investigationDates = [
            new Date(investigationDate),
            investigationDate,
        ]
        ...
        return {
            id: item.item.value,
            sex: item.hasOwnProperty('sexLabel')
                ? item.sexLabel.value
                : 'unknown',
        ...
        for (let i = 0; i < this.queryOutput.results.bindings.length; i++) {
            ...
            if (item.hasOwnProperty(this.plotFieldsLabels[plotByField])) {
                ...
                if (!repeatedWitch) {
                    ...
                    witch.mannerOfDeath = item.hasOwnProperty(
                        'mannerOfDeathLabel'
                    )
                    ...
                    if (item.hasOwnProperty('ritualObjects')) {
                        witch.ritualObjects = this.getSplitData(
                            item.ritualObjects.value
                        )
                    }
                    ...
                    if (!existingPlotBys.includes(newLocationPlotBy)) {
                        ...
                        newLocationsKeys.map((newLocationKey) => {
                            if (newLocationKey !== plotByField) {
                                if (
                                    newLocations[newLocationKey].locations
                                        .length !== 0
                                ) {
                                    let newLocation =
                                        newLocations[newLocationKey]
                                            .locations[0]
                                    let newCoordinates =
                                        newLocations[newLocationKey]
                                            .coordinates[0]
