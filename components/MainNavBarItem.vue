<template>
  <span class="dropdown block lg:inline-block">
    ...
    <nuxt-link :to="url" class="block xl:pt-1 xl:pb-1 mt-4 lg:inline-block lg:mt-0 text-gray-500 hover:text-black lg:mr-2 xl:mr-4" :class="{ 'menuBarItemActive' : ( isPageActive(url) || isSubpageActive(subpages))}" v-else>
      <slot></slot>
    </nuxt-link>
    <div v-if="subpages.length > 0" class="block dropdown-content lg:absolute lg:hidden lg:bg-gray-200 lg:shadow-xl text-gray-500">
      <a v-for="(page, index) in subpages" :href="page.url" class="block pt-3 pb-0 pl-2 pr-2 lg:p-2 hover:text-black" :class="{ 'subMenuItemActive' : isPageActive(page.url)}">{{ page.name }}</a>
    </div>
  </span>
</template>
